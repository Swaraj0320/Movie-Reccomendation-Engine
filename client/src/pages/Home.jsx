...
function Home() {
  ...
  // SKELETON LOADING - added 2026-07-14
  const skeletonCards = Array.from({ length: 8 }, (_, index) => <SkeletonCard key={index} compact />);
  const trendingSkeletonCards = Array.from({ length: 8 }, (_, index) => <SkeletonCard key={index} />);

  return <div className="mx-auto w-full max-w-[1600px] px-5 pb-5 pt-8 sm:px-8 sm:pt-10 lg:px-12"><section className="relative overflow-hidden border-y border-white/[0.09] py-8 sm:py-10"><div className="absolute inset-y-0 right-0 hidden w-1/2 isolate sm:block">{featuredMovie?.poster_path && !isLoading && <img src={posterUrl(featuredMovie.poster_path, "w780")} alt="" className="relative z-0 h-full w-full object-cover" />}<div aria-hidden="true" className="hero-image-overlay pointer-events-none absolute inset-0 z-10" /></div><div className="relative max-w-xl px-6 sm:px-8 lg:px-12">{isLoading ? <div aria-label="Loading featured movie" role="status"><div className="skeleton-shimmer mb-4 h-3 w-32 animate-pulse" /><div className="skeleton-shimmer h-10 w-4/5 animate-pulse sm:h-11" /><div className="skeleton-shimmer mt-3 h-10 w-3/5 animate-pulse" /><div className="mt-4 space-y-2"><div className="skeleton-shimmer h-4 w-full animate-pulse" /><div className="skeleton-shimmer h-4 w-4/5 animate-pulse" /></div><div className="skeleton-shimmer mt-6 h-9 w-32 animate-pulse" /></div> : <><p className="mb-4 text-[11px] font-semibold uppercase tracking-[0.22em] text-amber">Featured this week</p><h1 className="font-display text-[34px] font-medium leading-[1.08] tracking-[-0.04em] text-zinc-100 sm:text-[40px]">{featuredMovie ? featuredMovie.title : "Find the next film worth staying up for."}</h1><p className="mt-4 max-w-lg text-[15px] leading-6 text-zinc-400">{featuredMovie?.overview || "Explore a considered mix of Hindi favourites and global films selected from this week’s conversation."}</p><div className="mt-6 flex flex-wrap items-center gap-4"><Link to={featuredMovie ? `/movies/${featuredMovie.id}` : "/recommendations"} className="inline-flex items-center gap-2 bg-amber px-3.5 py-2 text-[12px] font-bold uppercase tracking-[0.12em] text-zinc-950 transition hover:bg-amber-soft">View film <ArrowRight size={14} /></Link><Link to="/recommendations" className="text-[12px] font-bold uppercase tracking-[0.14em] text-zinc-400 transition hover:text-zinc-100">Refine your picks</Link></div></>}</div></section><section className="pt-10"><div className="mb-5 flex items-end justify-between gap-4"><div><p className="text-[10px] font-medium uppercase tracking-[0.22em] text-amber">Selected this week</p><h2 className="mt-2 font-display text-[22px] font-medium tracking-[-0.025em] text-zinc-100">A global cut</h2></div><span className="hidden text-[11px] uppercase tracking-[0.14em] text-zinc-600 sm:block">Scroll to explore</span></div>{error && <p className="border border-red-900/60 bg-red-950/30 px-4 py-3 text-sm text-red-300">{error}</p>}{!isLoading && !error && movies.length === 0 && <p className="text-[15px] text-zinc-500">No trending films are available right now.</p>}{isLoading ? <div aria-label="Loading selected movies" role="status" className="hide-scrollbar -mx-5 flex gap-3 overflow-x-auto px-5 pb-2 sm:-mx-8 sm:px-8 lg:-mx-12 lg:px-12">{skeletonCards}</div> : selection.length > 0 && <div className="hide-scrollbar -mx-5 flex gap-3 overflow-x-auto px-5 pb-2 sm:-mx-8 sm:px-8 lg:-mx-12 lg:px-12">{selection.map((movie) => <EditorialMovieCard key={movie.id} movie={movie} compact />)}</div>}</section>{(isLoading || indiaTrending.length > 0) && <section className="border-t border-white/[0.09] pt-10"><div className="mb-6"><p className="text-[10px] font-medium uppercase tracking-[0.22em] text-amber">Trending now</p><h2 className="mt-2 font-display text-[22px] font-medium tracking-[-0.025em] text-zinc-100">India and beyond</h2><p className="mt-2 text-[15px] text-zinc-500">Hindi favourites selected separately from the global edit above.</p></div>{isLoading ? <div aria-label="Loading trending movies" role="status" className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8">{trendingSkeletonCards}</div> : <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8">{indiaTrending.map((movie) => <EditorialMovieCard key={movie.id} movie={movie} />)}</div>}</section>}<p className="mt-10 text-right text-xs font-normal text-zinc-400">Developed and Maintained by Siddhi & Swaraj</p></div>;
}
