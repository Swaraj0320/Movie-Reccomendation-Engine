...
function MovieDetails() {
  ...
  return <div className="page-shell"><p className="section-kicker">Film details</p>{error && <p className="mb-6 rounded-xl border border-red-900/60 bg-red-950/30 px-4 py-3 text-sm text-red-300">{error}</p>}<div className="grid gap-9 md:grid-cols-[280px_1fr]"><div className="aspect-[2/3] overflow-hidden rounded-xl border border-line bg-panel">{movie.poster_path ? <img src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} alt={movie.title} className="h-full w-full object-cover" /> : <div className="flex h-full items-end p-5 font-display text-2xl font-bold text-zinc-500">{movie.title}</div>}</div><section><h1 className="page-title">{movie.title}</h1><p className="mt-3 text-sm text-zinc-400">{movie.release_date?.slice(0, 4) || "Release date unavailable"} <span className="mx-2 text-zinc-700">•</span> TMDB {movie.vote_average?.toFixed?.(1) ?? "—"}/10</p><div className="mt-4 flex flex-wrap gap-2">{movie.genres.map((genre) => <span key={genre.id} className="rounded-full border border-line px-3 py-1 text-xs text-zinc-400">{genre.name}</span>)}</div><p className="mt-5 max-w-2xl leading-7 text-zinc-400">{movie.overview || "No overview is available for this movie."}</p><div className="mt-7 flex flex-wrap gap-3"><button onClick={watchTrailer} className="primary-button"><Play size={17} fill="currentColor" /> Watch trailer</button><button onClick={toggleWatchlist} disabled={isSaving} className="ghost-button">{isWatchlisted ? <Check size={17} className="text-amber" /> : <Bookmark size={17} />} {isWatchlisted ? "Remove from watchlist" : "Add to watchlist"}</button></div><div className="mt-10 border-t border-line pt-6"><p className="mb-2 text-sm font-semibold text-zinc-300">Your rating {rating ? `— ${rating}/10` : ""}</p><RatingStars value={rating} onChange={saveRating} /><p className="mt-2 text-xs text-zinc-500">Select a score from 1 to 10.</p></div></section></div><TrailerModal videoKey={videoKey} onClose={() => setVideoKey(null)} /></div>;
}
